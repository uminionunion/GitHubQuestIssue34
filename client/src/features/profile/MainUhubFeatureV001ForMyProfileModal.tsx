...
default:
  ...
  />
        ...
        // QUADRANTS MODAL - PAGE 1 REDESIGNED
const QuadrantsModal = ({ isOpen, onClose, stores, onSelectStore, user, mainStoreProducts, userStoreProducts, isLoadingProducts }) => {
  const [currentPage, setCurrentPage] = useState(1);
  ...
  const handleSubmitProduct = async (e: React.FormEvent) => {
    ...
    try {
      ...
      formDataObj.append('sku_id', formData.sku_id);
      formDataObj.append('url', formData.website_url);
      formDataObj.append('store_id', '0');
      ...
      setSuccessMessage('✓ Product added to Main Store!');
      setFormData({
        name: '',
        price: '',
        sku_id: '',
      ...
      // Refresh products after 1 second
      setTimeout(() => {
        setMyStoreView('list');
        setSuccessMessage('');
      }, 2000);
          ...
          <div className="grid grid-cols-2 gap-4 h-[70vh]">
                ...
                {isLoadingProducts ? (
                  ...
                  mainStoreProducts.map((p) => (
                    <div key={p.id} className="border rounded p-2 text-xs">
                      <p className="font-semibold">{p.name}</p>
                      {p.price && <p className="text-muted-foreground">${p.price.toFixed(2)}</p>}
                      {p.sku_id && <p className="text-muted-foreground">SKU: {p.sku_id}</p>}
                    </div>
                  ))
                ) : (
                  <div className="text-center text-muted-foreground py-4 text-sm">No products in main store</div>
                )}
              </div>
            </div>
            ...
            <div className="border rounded-lg p-4 flex flex-col h-full">
              <div className="flex justify-between items-center mb-3">
                ...
                {user && user.is_high_high_high_admin === 1 && (
                  <div className="flex gap-1">
                    ...
                    <Button
                      variant={myStoreView === 'add' ? 'default' : 'outline'}
                      size="sm"
                      onClick={() => setMyStoreView('add')}
                      className="text-xs"
                    >
                      + Add
                    </Button>
              ...
              <div className="flex-1 overflow-y-auto">
                {!user ? (
                  ...
                  <>
                    {myStoreView === 'list' ? (
                        ...
                        {isLoadingProducts ? (
                          ...
                          mainStoreProducts.map((p) => (
                            <div key={p.id} className="border rounded p-2 text-xs">
                              ...
                              {p.sku_id && <p className="text-muted-foreground">SKU: {p.sku_id}</p>}
                              {p.url && (
                                <a href={p.url} target="_blank" rel="noopener noreferrer" className="text-orange-400 hover:underline text-xs">
                                  Visit Website
                                </a>
                              )}
                            </div>
                      ...
                      <form onSubmit={handleSubmitProduct} className="space-y-3">
                        <div className="text-center bg-green-900 bg-opacity-30 border border-green-700 rounded p-2 mb-2">
                          <p className="text-xs font-semibold text-green-300">✓ You are HIGH-HIGH-HIGH Admin</p>
                          <p className="text-xs text-green-200">Adding to: Main Store (#0)</p>
                        </div>
                        ...
                        <div>
                          ...
                          <input
                            ...
                            placeholder="e.g., Uminion Union Card"
                            className="w-full px-2 py-1 text-xs border rounded bg-gray-800 text-white"
                            required
                          />
                        </div>
                        ...
                        <div>
                          <label className="text-xs font-semibold block mb-1">WooCommerce SKU</label>
                          <input
                            type="text"
                            name="sku_id"
                            value={formData.sku_id}
                            onChange={handleFormChange}
                            placeholder="e.g., UNION-CARD-001"
                            className="w-full px-2 py-1 text-xs border rounded bg-gray-800 text-white"
                          />
                          <p className="text-xs text-muted-foreground mt-1">Links to your WooCommerce product</p>
                        </div>

                        <div>
                          <label className="text-xs font-semibold block mb-1">Website Link (Optional)</label>
                          <input
                            type="url"
                            name="website_url"
                            ...
                            className="w-full px-2 py-1 text-xs border rounded bg-gray-800 text-white"
                          />
                          <p className="text-xs text-muted-foreground mt-1">Clicking image will open this link</p>
                        </div>
                        ...
                        <div>
                          ...
                          />
                          {formData.image && (
                            <p className="text-xs text-green-400 mt-1">✓ {formData.image.name} selected</p>
                          )}
                        </div>
                        ...
                        <Button
                          ...
                          className="w-full bg-orange-400 hover:bg-orange-500 text-black text-xs font-semibold"
                        >
                          {isSubmitting ? 'Adding...' : 'Add to Main Store'}
                        </Button>
                      </form>
                    )}
                  </>
                  ...
                  <div className="flex items-center justify-center h-full text-muted-foreground text-sm">
                    Only HIGH-HIGH-HIGH Admins can manage Main Store products
                  </div>
                )}
              </div>
            </div>
