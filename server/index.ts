...
app.get('/api/chat/unread-status', async (req: RequestWithUser, res: Response) => {
  try {
    if (!req.user?.id) {
...
app.post('/api/chat/mark-as-read', async (req: RequestWithUser, res: Response) => {
  try {
    if (!req.user?.id) {
      return res.status(401).json({ error: 'Not authenticated' });
    }
    ...
    // Generate room name (you may need to adjust based on your naming convention)
    const sisterUnionNum = String(chatroomNumber).padStart(2, '0');
    const roomName = `SisterUnion${sisterUnionNum}-chatroom-1`; // Adjust as needed
